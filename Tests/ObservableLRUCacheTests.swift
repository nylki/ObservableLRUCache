...
class ObservableLRUCacheTests: XCTestCase {
    func testCountLimit() {
        let cache = ObservableLRUCache<Int, Int>(countLimit: 2)
    ...
    func testCostLimit() {
        let cache = ObservableLRUCache<Int, Int>(totalCostLimit: 3)
    ...
    func testAdjustCountLimit() {
        let cache = ObservableLRUCache<Int, Int>(totalCostLimit: 2)
        cache.setValue(0, forKey: 0, cost: 1)
        cache.setValue(1, forKey: 1, cost: 1)
        cache.countLimit = 1
        XCTAssertNil(cache.value(forKey: 0))
